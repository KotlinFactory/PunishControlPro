...
public class ItemUtils {
  ...
  static {
    try {
      ...
      final Class<?> obcCraftItemStackClass =
      ...
      final Class<?> nmsItemStackClass = ReflectionUtil.getClass("{nms}.ItemStack");
      ...
      nbtCompoundSetMethod =
          nmsTagCompoundClass.getMethod(
              "set", String.class, ReflectionUtil.getClass("{nms}.NBTBase"));
  ...
  public static int getFreeSlots(final Inventory inv) {
    final ItemStack[] a = inv.getContents();
    int c = 0;
    for (final ItemStack ic : a) {
  ...
  public static int getFirstSlot(final Inventory inv, final String unique) {
    ...
    for (int i = 0; i < a.length; i++) {
      final ItemStack ic = a[i];
      if (ic != null) {
        final String n = getHiddenString(ic);
  ...
  public static ItemStack removeLore(final ItemStack is) {
  ...
  public static ItemStack setSkullAndName(final ItemStack is, final String name) {
  ...
  public static void setSkullOwner(final ItemStack stack, final String skullOwner) {
     ...
     */
    final UUID uuid = Providers.playerProvider().getUUID(skullOwner);
    Valid.notNull(uuid, "No player found for UUID '" + uuid + "'");
    setSkullTexture(stack, Providers.textureProvider().getSkinTexture(uuid));
  }

  public static int count(final Inventory inv, final String hidden) {
    int r = 0;
    for (final ItemStack is : inv.getContents()) {
      if (is != null) {
        if (hidden.equalsIgnoreCase(getHiddenString(is))) {
          r += is.getAmount();
        }
      }
    }
  ...
  public static int count(final Player p, final Material material) {
    int r = 0;
  ...
  public static int count(final Player p, final Predicate<ItemStack> match) {
    int r = 0;
    final Inventory inv = p.getInventory();
    for (final ItemStack is : inv.getContents()) {
  ...
  public static int count(final Player p, final String hidden) {
    return count(p.getInventory(), hidden);
  }

  public static void remove(final Player p, final int toRemove, final Material material) {
    final Inventory inv = p.getInventory();
    int removed = 0;
  ...
  public static void remove(final Player p, final int toRemove, final Predicate<ItemStack> match) {
    final Inventory inv = p.getInventory();
    int removed = 0;
    for (int i = 0; i < inv.getSize(); i++) {
      final ItemStack is = inv.getItem(i);
  ...
  public static void remove(final Player p, final String hidden) {
    final Inventory inv = p.getInventory();
    for (final ItemStack is : inv) {
      final String s = is != null && is.getType() != Material.AIR ? getHiddenString(is) : null;
  ...
  public static void remove(final Player p, final String hidden, final int toRemove) {
    final Inventory inv = p.getInventory();
    int removed = 0;
    for (int i = 0; i < inv.getSize(); i++) {
      final ItemStack is = inv.getItem(i);
      ...
      final String h = getHiddenString(is);
      if (hidden.equals(h)) {
        int a = is.getAmount();
        while (a > 0 && removed < toRemove) {
          removed++;
  ...
  public static ItemStack setNBTTags(final ItemStack item, final NBTTagPair... values) {
    try {
      final Object nmsStack = asNMSCopyMethod.invoke(null, item);
      Object tag = null;
  ...
  public static ItemStack addGlow(final ItemStack item) {
    try {
      return setNBTTags(
          item, new NBTTagPair("ench", ReflectionUtil.getClass("{nms}.NBTTagList").newInstance()));
    } catch (final Exception e) {
      e.printStackTrace();
      return null;
    }
  ...
  public static ItemStack unbreakable(final ItemStack is) {
    try {
      return setNBTTags(
          is,
          new NBTTagPair(
  ...
  public static ItemStack removeGlow(final ItemStack is) {
    try {
      final Object nmsStack = asNMSCopyMethod.invoke(null, is);
      Object tag = null;
  ...
  public static ItemStack first(final Inventory inv, final Predicate<ItemStack> match) {
    for (final ItemStack is : inv.getContents()) {
      if (is != null && match.test(is)) {
        return is;
      }
  ...
  public static ItemStack first(final Inventory inv, final Material mat) {
    final int slot = inv.first(mat);
    if (slot == -1) {
      return null;
    }
